...
class MediaPlayer:
    def __init__(self, window):
        ...
        self.song_photo.place(x=20, y=50)
        ...
        self.delete_all_icon = self.delete_all_icon.resize(
        ...
        self.add_song_icon = self.add_song_icon.resize(
            (30, 30), Image.ANTIALIAS)
        self.add_song_icon = ImageTk.PhotoImage(self.add_song_icon)


        tk.Label(self.root, text="", background=background,height=9, width=91, bd=5, relief="ridge").place(x=4.5, y=397)

        self.songs_list = tk.Listbox(
            self.root, width=30, height=18, bg="black", fg="white", relief="flat", selectbackground="grey")
        self.songs_list.place(x=520, y=70)

        self.time_elapsed_label = tk.Label(self.root, text="00:00", fg="black", background=background, activebackground=background, padx=5)
        self.time_elapsed_label.place(x=10, y=408)

        self.music_duration_label = tk.Label(
        self.root, text="00:00", fg="black", background=background, activebackground=background, padx=15)
        self.music_duration_label.place(x=520, y=408)

        self.progress_scale = ttk.Scale(self.root, orient="horizontal", style='TScale', from_=0, length=470,command=self.progress_scale_moved, cursor='hand2')
        self.progress_scale.place(x=55, y=408)

        self.speaker_button = tk.Button(self.root, image=self.speaker_icon, command="command", cursor='hand2', bd=0,background=background, activebackground=background)
        self.speaker_button.place(x=575, y=402)

        self.vol_scale = ttk.Scale(self.root, from_=0, to=100, orient="horizontal", command=self.volume, cursor="hand2")
        self.vol_scale.place(x=635, y=410)

        self.shuffle_button = tk.Button(self.root, image=self.shuffle_icon, command=self.shuffle_songs, cursor='hand2', bd=0,background=background, activebackground=background)
        self.shuffle_button.place(x=15, y=462)

        self.repeat_button = tk.Button(self.root, image=self.repeat_icon, command=self.repeat_song, cursor='hand2', bd=0,background=background, activebackground=background)
        self.repeat_button.place(x=85, y=462)
        ...
        self.play_button.place(x=270, y=450)

        self.next_button = tk.Button(self.root, image=self.next_icon, command=self.next_song, cursor='hand2', bd=0,background=background, activebackground=background)
        self.next_button.place(x=472, y=460)

        self.previous_button = tk.Button(self.root, image=self.previous_icon, command=self.previous_song, cursor='hand2', bd=0,background=background, activebackground=background)
        self.previous_button.place(x=190, y=460)

        self.stop_button = tk.Button(self.root, image=self.stop_icon, command=self.stop_song, cursor='hand2', bd=0,background=background, activebackground=background)
        self.stop_button.place(x=370, y=450)

        self.auto_play_button = tk.Button(self.root, image=self.auto_play_icon, command=self.auto_play_song, cursor='hand2', bd=0,background=background, activebackground=background)
        self.auto_play_button.place(x=610, y=470)
        ...
        self.status.place(x=5,y=550,width=740)
...
if __name__ == '__main__':
    ...
    window.maxsize(width=750,height=580)
    window.minsize(width=750,height=580)
